...
GeoXpl opens on a map of southeastern Australia. Enter a feature name, select **River** or **Valley**, and search. Names are case-insensitive. With **River** selected, the trailing word "River" is optional: "Loddon" and "Loddon River" reuse the same request and results. Existing duplicate spellings prefer the request with active geometry, then a resolved result; historical requests and evidence are retained. Distinct same-named rivers still require selection. The catalogue starts empty. Every new request creates a persistent job; the browser waits up to five seconds, then invites the user to return later while work continues. Repeated searches reuse the same job.
...
Legacy duplicate requests without active geometry can be linked to an equivalent resolved request via authenticated `POST /api/admin/jobs/:id/supersede` with `{ "replacementJobId": "..." }`. The old request gains `superseded_by`, status/phase `superseded`, and an audit record of its former state; reports, imports and derivations are retained. Active attempts, unrelated names/types and requests with active geometry cannot be superseded. Old public job URLs resolve to the replacement, while administration labels the old request and reports as superseded, links to the current map result, and prevents retries/research/settings changes. Public map links accept `/?job=<job-id>` without creating a new search or processing job.
...
An exception supports two named streams forming a differently named watercourse. Authenticated feature settings can contain `formationJunctions`, each with `nodeId`, `tributaryHydroId`, `joiningHydroId`, `downstreamHydroId`, `joiningName`, `downstreamName`, an HTTPS `evidenceUrl`, and a review `note`. This applies only to the supported BoM network and requires exactly two incoming streams, the reviewed names/IDs, a classified junction, consistent downstream links, known directions, and coincident geometry. Missing or changed evidence cannot approve the endpoint. The naming reference and supporting network records remain in provenance; neither joining nor downstream geometry is added to the requested feature. A verified downstream junction does not approve an uncertain headwater. Ordinary settings saves retain these reviews; changing aliases clears omitted reviews. Explicit `formationJunctions: []` removes them.

For uncertain named starts, separately named upstream tributaries outside the approved query/aliases are identified in diagnostics and excluded from automatic upstream estimates. Unnamed possibilities can still be shown dotted, remain unverified, and never add to recorded length. This does not choose a headwater simply because only one unnamed possibility remains.
