...
export async function publicJson(input, maxBytes = 20_000_000) {
  ...
  try {
    const response = await fetch(url, { dispatcher: agent, redirect: 'error', signal: AbortSignal.timeout(45000), headers: { 'User-Agent': 'GeoXpl/0.1 (local geographic research)', Accept: 'application/json' } });
