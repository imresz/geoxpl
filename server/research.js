...
export async function research(job, store, reason) {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({ model: process.env.OPENAI_MODEL, store: false, max_output_tokens: 4000, tools: [{ type: 'web_search' }],
      ...
      input, text: { format: { type: 'json_schema', name: 'geoxpl_research', strict: true, schema: z.toJSONSchema(reportSchema) } } })
  });
  if (!response.ok) throw new Error(`AI service returned HTTP ${response.status}. Check model and credentials.`);
